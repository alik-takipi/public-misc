...
public class AlikApp
{
	public static void main(String[] args) throws InterruptedException
	{
		...
		if (args.length > 1 && args[1].equals("justNew"))
		{
			...
			{
				new Thread(new Delegator3(new ThrowerRegularNew504())).start();
		...
		if (args.length > 1 && args[1].equals("justPack"))
		{
			for (int i = 0 ; i < retries ; ++i)
			{
				...
				new Thread(new ThrowerPack504()).start();
		...
		for (int i = 0 ; i < retries ; ++i)
		{
			...
			// Regular
			new Thread(new Delegator3(new ThrowerRegular504())).start();
...
//        new Thread(new ThrowerPack()).start();
			...
			// Mommy pack
			new Thread(new Delegator1(new ThrowerMommy504())).start();
			new Thread(new Delegator2(new ThrowerMommy504())).start();
